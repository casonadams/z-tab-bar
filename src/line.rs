...
fn left_more_message(tab_count_to_the_left: usize, palette: Palette, separator: &str) -> LinePart {
  ...
  let left_separator = style!(palette.green, palette.orange).paint(separator);
  let more_styled_text = style!(palette.black, palette.orange).paint(more_text);
  let right_separator = style!(palette.orange, palette.green).paint(separator);
  let more_styled_text = ANSIStrings(&[left_separator, more_styled_text, right_separator]).to_string();
  LinePart {
...
fn right_more_message(tab_count_to_the_right: usize, palette: Palette, separator: &str) -> LinePart {
...
fn tab_line_prefix(session_name: Option<&str>, palette: Palette, cols: usize) -> Vec<LinePart> {
  let prefix_text_len = 0;
  let mut parts = vec![];
  if let Some(name) = session_name {
    let name_part = format!("[{}] ", name);
    let name_part_len = name_part.width();
    let name_part_styled_text = style!(palette.black, palette.green).paint(name_part);
